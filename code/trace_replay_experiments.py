...
def run(trace,m,depth,seed):
 rng=random.Random(seed);classes=sorted({r['class'] for r in trace});queues=defaultdict(deque);order=deque(classes);pending=deque(trace);inflight=[];done=[];now=0.;peak=0
 def admit():
  nonlocal peak
 ...
 while pending or any(queues.values()) or inflight:
  admit()
  if inflight and (len(inflight)>=depth or (not any(queues.values()) and (not pending or pending[0]['arrival_cycle']>inflight[0][0]))):
   finish,batch=inflight.pop(0);now=max(now,finish);done.extend((t,now) for t in batch);continue
  picked=None
  ...
  wire=frames*FRAME_WIRE+TAG+CTRL+failed*(FRAME_WIRE+TAG+CTRL);service=wire/W+VERIFY+failed*64
  inflight.append((now+service,batch));inflight.sort(key=lambda x:x[0])
