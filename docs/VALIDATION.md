...
`code/baseline_comparison.py` adds a non-authenticated selective-repeat ARQ reference so the security cost is visible rather than hidden.

| Scheme | Security | Mean goodput | Mean p99 latency |
|---|---|---:|---:|
| Selective-repeat ARQ | none | 0.8925 | 51.8 cycles per frame |
| ShieldLink Mode A | per-frame AEAD | 0.8563 | 83.3 cycles per frame |
| Mode B flush-all | epoch AEAD | 0.7175 | 4020.5 cycles per epoch head |
| Mode B-SR | epoch AEAD | 0.9155 | 1328.4 cycles per epoch head |

Mode B-SR can exceed the no-authentication per-frame baseline in normalized goodput because one tag and one commit exchange are amortized across the epoch. The latency units are not identical, so the table must not be interpreted as a direct frame-latency ranking. Results are in `data/baseline_comparison.csv`.

## 3. Adaptive epoch-size ablation
...
## 4. Synthetic mixed-workload trace replay
...
## 5. RTL corrections and regression tests
...
## 7. Synthesis, timing proxy, and CI
